            ...
            <div className="space-y-3">
              {CAMP_TERMS.map((term) => (
                <div
                  ...
                  className={`p-5 rounded-xl border-2 cursor-pointer transition-all flex items-center justify-between ${formData.term === term ? "border-primary bg-primary/5" : "border-border hover:border-primary/50"
                    }`}
                >
        ...
        {step === 2 && (
          <div className="space-y-8 animate-fade-up">
            ...
            <div>
              ...
              <div className="mt-3">
                <Label htmlFor="heardAboutUsOther" className="h-6 flex items-center">
                  Máte slevový kupón?
                </Label>
                <Input disabled={formData.brnoSubsidy} id="couponCode" value={formData.couponCode} onChange={(e) => updateForm("couponCode", e.target.value)} placeholder="Kód slevového kupónu (nepovinné)" className="mt-1" />
                <p className="text-xs text-muted-foreground">
                  * Slevový kupón nelze využít v kombinaci s finančním příspěvkem města Brna. <a href="#kupony" className="text-primary hover:underline">Více o kupónech</a>.
                </p>
              </div>
            </div>
